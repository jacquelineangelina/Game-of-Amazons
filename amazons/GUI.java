...
class GUI extends TopLevel implements View, Reporter {
    ...
    GUI(String title) {
        ...
        addMenuButton("Player->auto black", this::newAutoManual);
        addMenuButton("Player->auto white", this::newAutoManual);
        addMenuButton("Player->manual black", this::newAutoManual);
        addMenuButton("Player->manual white", this::newAutoManual);
        addMenuButton("Game->New", this::newGame);
        _widget = new BoardWidget(_pendingCommands);
        add(_widget, new LayoutSpec("y", 1,
                           "height", 1,
                           "width", 3));
    }
    ...
    /** Response to amazon COMMANDS returns command. */
    private void newAutoManual(String commands) {
        _pendingCommands.offer(commands.substring(commands.indexOf(">") + 1));
    }
    ...
    String readCommand() {
        try {
            ...
            while (true) {
                ...
                if (cmnd.matches(Square.SQ)) {
                    counter += 1;
                    move += cmnd + " ";
                    if (counter == 3) {
                        move = move.trim();
                        if (_widget.get().isLegal(mv(move))) {
