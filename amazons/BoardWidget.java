...
class BoardWidget extends Pad {
        ...
        for (int i = 0; i < 10; i += 2) {
            for (int j = 0; j < 10; j += 1) {
                if (j % 2 == 0) {
                    ...
                    g.setColor(DARK_SQUARE_COLOR);
                    g.fillRect(cx(i + 1), cy(j), SQUARE_SIDE, SQUARE_SIDE);
                }
            }
        }
    }
    ...
    public synchronized void paintComponent(Graphics2D g) {
        ...
        for (int i = 0; i < 10; i += 1) {
            for (int j = 0; j < 10; j += 1) {
                if (_board.get(i, j) == BLACK) {
                    drawQueen(g, sq(i, j), BLACK);
                } else if (_board.get(i, j) == WHITE) {
                    drawQueen(g, sq(i, j), WHITE);
                } else if (_board.get(i, j) == SPEAR) {
